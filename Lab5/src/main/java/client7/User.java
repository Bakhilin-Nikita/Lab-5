...
import client.SerializationManager;
...
public class User {
    ...
    private client.SendObject sendObject;
    private String host;
    private int port;
    ...
    private static client.CurrentUser authorize() throws NoSuchAlgorithmException, IOException, SQLException {
        ...
        client.CurrentUser now = new CurrentUser(login.getUserID());
        return now;
    }
    ...
    private void sendMessage(String message) throws ClassNotFoundException, IOException, InterruptedException {
        ...
        if (message.equals("show")) {
            try{
            if (client.SerializationManager.deserialize(receivingPacket.getData()).toString().trim().equals("[]"))
                System.out.println("Коллекция пустая!");
            else
                System.out.println(client.SerializationManager.deserialize(receivingPacket.getData()).toString().trim());
        ...
        while (!flag) {
            ...
            if (!message.isEmpty()) {
                if (message.equals("exit")) {
                    ...
                    sender.sendMessage("auth "+identifier);
                    sender.sendObject = new client.SendObject(sender.getLabs());
    ...
    private void sendLabWorkObject(BufferedReader b) throws IOException {
        ...
        socket.connect(address, port);
        receivingDataBuffer = new byte[5000];
        byte[] data = client.SerializationManager.serialize(sendObject.getLabWork());
        // Создайте UDP-пакет
        sendingPacket = new DatagramPacket(data, data.length, address, port);
        // Отправьте UDP-пакет серверу
        socket.send(sendingPacket);
    ...
    private LinkedList<LabWork> getLabs() throws IOException {
        ...
        try {
            if (client.SerializationManager.deserialize(receivingPacket.getData()) == null)
                return null;
            else {
            ...
            while (flag) {
                ...
                while (command != null) {
                    ...
                    if (ex[0].equals("execute_script")) {
                        ...
                        sendObject = new client.SendObject(getLabs());
                        sendMessage(command);
